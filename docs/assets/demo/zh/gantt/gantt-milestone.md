...
甘特图中的里程碑(Milestone)用于标记项目中的重要时间节点。它不同于普通任务,通常以一个醒目的菱形符号表示,而不是时间跨度的任务条。本示例展示了里程碑的基础用法,帮助您更好地标记和追踪项目关键节点。

## 关键配置

- `Gantt#taskBar.milestoneStyle`: 配置里程碑的样式,如大小、颜色等
- 数据中设置 `type: 'milestone'` 来标记里程碑任务
- 里程碑任务只需要设置 `start` 时间即可
