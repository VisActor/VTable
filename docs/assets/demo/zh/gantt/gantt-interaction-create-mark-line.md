...
甘特图创建 markLine。
配置项分别为`IMarkLineCreateOptions`和`IMarkLine`。
demo 中展示的创建 markline 的弹窗需要自己去实现，代码里只提供了创建 markline 的点击回调。

## 关键配置

- `Gantt`
- `Gantt#IMarkLineCreateOptions` 创建 markLine 配置项
- `Gantt#IMarkLine` markLine 配置项
