...
- enableCheckboxCascade: 默认true 在全局使用，用于开启复选框级联，必须与 cellType: 'checkbox'和 tree:true 配合使用，可实现父子元素勾选同步
- enableHeaderCheckboxCascade: 默认true 在全局使用，用于开启表头复选框级联，必须与 cellType: 'checkbox'和 tree:true 配合使用，可实现表头和body单元格勾选同步
