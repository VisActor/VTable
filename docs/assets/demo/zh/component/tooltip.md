...
在该示例中，配置了 tooltip.isShowOverflowTextTooltip 为 true，超长显示不了被省略的文字被 hover 时将提示出来。
同时通过监听`mouseenter_cell`事件，鼠标移入符合提示条件【第一列订单号】的单元格时，调用接口 showTooltip 来显示提示信息。
