...
заголовок: список таблица данные filtering
обложка: https://lf9-dp-fe-cms-tos.byteorg.com/obj/bit-cloud/Vтаблица/preview/список-таблица-filter.gif
ссылка: данные_analysis/список_таблица_данныеAnalysis
---

# список таблица данные filtering

The базовый таблица sets filtering through the интерфейс updateFilterRules, supporting значение filtering и функция filtering.

## Ключевые Конфигурации

- `списоктаблица`
- `updateFilterRules` sets или updates filtering данные rules
  ...
  .then(данные => {
    ...
    функция createFilterElement(values, curValue, поле, positonRect) {
      ...
      выбрать.addсобытиесписокener('change', () => {
        ...
        removeFilterElement();
        // update header иконка
        columns.forEach(col => {
          if (col.поле === поле) {
            col.headerиконка = 'filtered';
          }
        });
        таблицаInstance.updateColumns(columns);
      });
    }
