...
категория: примеры
группа: гантт
заголовок: гантт Task Hour Unit
обложка: https://lf9-dp-fe-cms-tos.byteorg.com/obj/bit-cloud/Vтаблица/гантт/гантт-hour.png
ссылка: гантт/introduction
опция: гантт#timelineHeader.scales
---

# гантт Task Hour Unit

Support configuration из time scale unit('day' | 'week' | 'month' | 'quarter' | 'year' | 'hour' | 'minute' | 'second')
...
const columns = [
  {
    headerType: 'флажок', //指定表头单元格显示为复选框
    cellType: 'флажок', //指定body单元格显示为复选框
    поле: 'check',
    checked: false,
    ширина: 50
  },
