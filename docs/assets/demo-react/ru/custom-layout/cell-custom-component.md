...
категория: примеры
группа: компонент
заголовок: cell custom компонент
...
# cell custom компонент

Like customLayout, you can use react компонентs to customize layout. For details, please refer to [Пользовательский Компонентs](../../guide/custom_define/react-custom-компонент)
