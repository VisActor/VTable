...
demo 可以参考[demo](/vtable/demo/search/search-component)
...
const search = new SearchComponent({
  table: tableInstance,
  autoJump: true, // 搜索完成后自动跳转到第一个搜索结果
  autoScroll: {
    // 配置滚动行为
    enableTableScroll: true, // 启用表格内滚动
    enableViewportScroll: true // 启用页面滚动
  }
});
```
...
search 组件提供`search`方法，用于搜索单元格的内容，高亮并返回搜索结果：
...
search 组件提供`next`和`prev`方法，用于跳转搜索结果：
...
| autoScroll | object | 自动滚动配置：`{enableTableScroll?: boolean, enableViewportScroll?: boolean}`。`enableTableScroll` 控制是否在单元格不在表格可视区域内时自动滚动表格内部；`enableViewportScroll` 控制是否在单元格不在页面可视区域内时自动滚动整个页面 |
...
```js
{
  ...
  search(str: string): {
      ...
      results: {
          ...
          row?: number;
          value?: string;
          indexNumber?: number[]; // 树形结构时使用，表示节点路径
      }[];
  }
}
```
