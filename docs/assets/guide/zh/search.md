...
| ~~focuseHighlightCellStyle~~, focusHighlightCellStyle | ICellStyle | 当前目标搜索结果的高亮样式 |
...
| enableViewportScroll | boolean | 当目标单元格不在 viewport 内时，是否滚动它的父级滚动元素，保证在可视区域内 |
| callback | (queryResult: QueryResult, table: IVTable) => void | 搜索完成后的回调 |

注意

- `highlightCellStyle`和`focusHighlightCellStyle`是两个不同的样式，前者用于高亮所有搜索结果，后者用于高亮当前目标搜索结果，这两个样式都是对搜索结果的目标单元格整体生效，目前还不支持对单元格内的部分文字进行高亮。
