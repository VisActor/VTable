...
可以通过`taskBar.barStyle`配置来设置普通任务的样式，通过`taskBar.projectStyle`配置项目任务的样式，通过`taskBar.milestoneStyle`配置里程碑的样式：
...
可以通过`taskBar.selectedBarStyle`配置任务条选中时的样式：
...
对于更复杂的自定义需求，可以通过`taskBar.customLayout`完全控制任务条的渲染：
