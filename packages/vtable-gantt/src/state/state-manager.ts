...
import { findRecordByTaskKey, getSubTaskRowIndexByRecordDate, getTaskIndexsByTaskY } from '../gantt-helper';
...
import { updateLinkLinePoints } from '../scenegraph/dependency-link';
  ...
  endMoveTaskBar() {
    ...
    if (this._gantt.parsedOptions.tasksShowMode === TasksShowMode.Sub_Tasks_Arrange) {
      ...
      if (Math.abs(Math.round(deltaY / this._gantt.parsedOptions.rowHeight)) >= 1) {
        ...
        resizeOrMoveTaskBar(
          target,
          targetEndX - (target as Group).attribute.x,
...
function resizeOrMoveTaskBar(target: GanttTaskBarNode, dx: number, dy: number, newWidth: number, state: StateManager) {
  ...
  for (let i = 0; i < vtable_gantt_linkedTo?.length; i++) {
    ...
    let diffY: number;
    if (state._gantt.parsedOptions.tasksShowMode === TasksShowMode.Sub_Tasks_Inline) {
      ...
      state._gantt.parsedOptions.tasksShowMode === TasksShowMode.Sub_Tasks_Separate ||
      ...
      ({
        ...
        endDate: linkedToTaskEndDate,
        taskDays: linkedToTaskTaskDays
      } = state._gantt.getTaskInfoByTaskListIndex(linkedToTaskShowIndex));
      ({
        startDate: linkedFromTaskStartDate,
        endDate: linkedFromTaskEndDate,
        taskDays: linkedFromTaskTaskDays
      } = state._gantt.getTaskInfoByTaskListIndex(linkedFromTaskShowIndex));
    }
