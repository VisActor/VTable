<template>
  <vue-list-table :options="option" :records="records" ref="tableRef">
    ...
    <ListColumn :field="'bloggerName'" :title="'anchor nickname'" :width="330"
      ...
      <template #customLayout="{ table, row, col, rect, record, height, width  }">
        <Group :height="height" :width="width" display="flex" flexDirection="row" flexWrap="nowrap">
          <Group :height="percentCalc(100)" :width="60" display="flex" flexDirection="column" alignItems="center"
            justifyContent="space-around">
            <Image id="icon0" :width="50" :height="50" :image="record.bloggerAvatar" :cornerRadius="25"
              @mouseEnter="(e) => { console.log(e); }" />
          </Group>
          <Group :height="height" :width="percentCalc(100, -60)" display="flex" flexDirection="column"
            flexWrap="nowrap">
            <Group :height="percentCalc(50)" :width="percentCalc(100)" display="flex" alignItems="flex-end">
              ...
              <Image id="location" image="https://lf9-dp-fe-cms-tos.byteorg.com/obj/bit-cloud/VTable/location.svg"
                :width="15" :height="15" :boundsPadding="[0, 0, 0, 10]" />
            ...
            <Group :height="percentCalc(50)" :width="percentCalc(100)" display="flex" alignItems="center">
              <Tag v-for="tag in record?.tags" :text="tag"
                :textStyle="{ fontSize: 10, fontFamily: 'sans-serif', fill: 'rgb(51, 101, 238)' }"
                :panel="{ visible: true, fill: '#f4f4f2', cornerRadius: 5 }" :space="5" :boundsPadding="[0, 0, 0, 5]" />
            </Group>
          </Group>
        </Group>
      </template>
    ...
    <ListColumn :field="'fansCount'" :title="'fansCount'" :fieldFormat="rec => rec.fansCount + 'w'"
      :style="{ fontFamily: 'Arial', fontSize: 12, fontWeight: 'bold' }" />

    <ListColumn :field="'worksCount'" :title="'worksCount'"
      :style="{ fontFamily: 'Arial', fontSize: 12, fontWeight: 'bold' }" />

    <ListColumn :field="'viewCount'" :title="'viewCount'" :fieldFormat="rec => rec.fansCount + 'w'"
      :style="{ fontFamily: 'Arial', fontSize: 12, fontWeight: 'bold' }" />

    <ListColumn :field="'viewCount'" :title="'viewCount'" :fieldFormat="rec => rec.viewCount + 'w'"
      :style="{ fontFamily: 'Arial', fontSize: 12, fontWeight: 'bold' }" />
      ...
      <template #customLayout="{ table, record, height, width }">
        <Group :height="height" :width="width" display="flex" flexDirection="column" justifyContent="center">
          <Group display="flex" flexDirection="row" flexWrap="nowrap" :boundsPadding="[5, 0, 5, 10]"
            ...
            <CheckBox :text="{ text: 'like', fontSize: 12 }" :spaceBetweenTextAndIcon="2" :boundsPadding="[0, 10, 0, 0]"
              @checkbox_state_change="handleCheckBoxStateChange($event)" />
            <CheckBox :text="{ text: 'collect', fontSize: 12 }" :spaceBetweenTextAndIcon="2"
              @checkbox_state_change="handleCheckBoxStateChange($event)" />
          </Group>
          <Group display="flex" flexDirection="row" flexWrap="nowrap" :boundsPadding="[5, 0, 5, 10]">
            <Text :text="'type: '" :fontSize="12" :boundsPadding="[0, 10, 0, 0]" />
            <Radio :text="{ text: 'normal', fontSize: 12 }" :checked="radio_state" :spaceBetweenTextAndIcon="2"
              :boundsPadding="[0, 10, 0, 0]" @radio_checked="() => {console.log(table.scenegraph)}" />
            <Radio :text="{ text: 'special', fontSize: 12 }" :checked="radio_state" :spaceBetweenTextAndIcon="2"
              @radio_checked="handleRadioChecked($event)" />
          </Group>
        </Group>
      </template>
    </ListColumn>
...
const handleRadioChecked = (e) => {
  radio_state.value = !radio_state.value;
  console.log('Radio state changed:',e);
} 
