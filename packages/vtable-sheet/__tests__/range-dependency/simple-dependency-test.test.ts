...
describe('HyperFormula Dependency Tracking - Understanding the Issue', () => {
  let hf: HyperFormula;
  ...
  test('should demonstrate the getCellDependents behavior with ranges', () => {
    ...
    hf.setCellContents({ sheet: 0, row: 1, col: 3 }, [['10']]); // D2
    hf.setCellContents({ sheet: 0, row: 2, col: 3 }, [['20']]); // D3
    hf.setCellContents({ sheet: 0, row: 1, col: 4 }, [['=SUM(D2:D3)']]); // E2

    console.log('\n=== Test Case: SUM(D2:D3) ===');
    console.log('D2 value:', hf.getCellValue({ sheet: 0, row: 1, col: 3 }));
    console.log('D3 value:', hf.getCellValue({ sheet: 0, row: 2, col: 3 }));
    console.log('E2 formula:', hf.getCellFormula({ sheet: 0, row: 1, col: 4 }));
    console.log('E2 value:', hf.getCellValue({ sheet: 0, row: 1, col: 4 }));

    // This is what you're experiencing - getCellDependents returns the RANGE, not the dependent cells
    const d2Dependents = hf.getCellDependents({ sheet: 0, row: 1, col: 3 });
    console.log('\nD2 dependents (what you get):', JSON.stringify(d2Dependents, null, 2));

    const d3Dependents = hf.getCellDependents({ sheet: 0, row: 2, col: 3 });
  ...
  test('should compare individual vs range reference behavior', () => {
    ...
    const a1Dependents = hf.getCellDependents({ sheet: 0, row: 0, col: 0 });
    ...
    hf.setCellContents({ sheet: 0, row: 0, col: 1 }, [['10']]);
    hf.setCellContents({ sheet: 0, row: 1, col: 1 }, [['20']]);
    hf.setCellContents({ sheet: 0, row: 2, col: 1 }, [['=SUM(B1:B2)']]);

    const b1Dependents = hf.getCellDependents({ sheet: 0, row: 0, col: 1 });
