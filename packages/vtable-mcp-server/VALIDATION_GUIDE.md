...
### 1. 基础验证（推荐）
```bash
npm run validate:simple  # 快速验证所有核心功能
```

### 2. 完整验证
```bash
npm run validate         # 完整验证流程（包含边界测试）
```
...
wscat -c ws://localhost:3001/mcp?session_id=test
