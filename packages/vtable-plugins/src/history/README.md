...
本文档描述 `HistoryPlugin` 的核心逻辑、事件到命令的映射、与 vtable-sheet 公式系统的交互点、如何在 vtable / vtable-sheet 中接入，以及已知的边界与排查路径。
...
- **Command**：最小回放单元，见 [types.ts](./types.ts)
- **Transaction**：一次 undo/redo 的原子操作；内部包含多个 commands
- **Snapshot**：用于结构性操作的“变更前快照”，见 [snapshot.ts](./snapshot.ts)
- **Formula event**：vtable-sheet 的 `formula_added` 事件，用于记录公式编辑

## 事件与命令映射

入口在 [history-plugin.ts](./history-plugin.ts) 的 `run()`：
...
回放入口在 [replay.ts](./replay.ts) 的 `replayCommand()`。
...
- `sheetKey`：优先通过 `workSheetInstances` 反查 `tableInstance`，否则回退 active sheet；见 [sheet-key.ts](./sheet-key.ts)
- 回放屏蔽：回放期间会置 `isReplaying`；vtable-sheet workbook 回放会置 `__workbookHistoryReplaying`，HistoryPlugin 会直接跳过采集
...
该插件实现了 VTable 的插件协议（`pluginsDefinition.IVTablePlugin`），在 ListTable 上工作。
...
使用时直接从插件实例调用 undo/redo（不同项目对 pluginManager 的访问方式可能不同）：
...
## 在 VTableSheet 中使用（与 WorkbookHistoryManager 融合）

vtable-sheet 有“工作簿层”的撤销恢复（跨 sheet 的新增/删除/重命名/排序 + 单 sheet 内的表格编辑）。
推荐做法是：

1) 表格层仍由 HistoryPlugin 生成 table transaction（`HistoryTransaction`）
2) 通过 `onTransactionPushed` 把该 transaction 上抛给 workbookHistoryManager，形成工作簿级别的 undo/redo
3) 快捷键（Ctrl/Cmd+Z/Y）由 workbookHistoryManager 接管，HistoryPlugin 只负责采集与回放 table transaction

对应实现参考 vtable-sheet 的插件注入逻辑：

- `onTransactionPushed` 透传：`packages/vtable-sheet/src/core/table-plugins.ts`
- 工作簿历史：`packages/vtable-sheet/src/managers/workbook-history-manager.ts`
