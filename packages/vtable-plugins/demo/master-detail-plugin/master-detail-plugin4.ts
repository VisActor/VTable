// 改case主要测试 主从表和CellType为text，image，video，progressbar，sparkline，link，chart的适配
...
function attachChildren(records: unknown[]): unknown[] {
  if (!Array.isArray(records)) {
    return records as unknown[];
  }
  return records.map((rec: unknown, idx: number) => {
    // 随机生成 1-3 条子任务
    const childrenCount = 1 + (idx % 3);
    const children = new Array(childrenCount).fill(0).map((_, i) => ({
      taskName: `任务 ${i + 1}`,
      progress: Math.floor(((i + 1) * 30) % 100)
    }));
    return Object.assign({}, rec, { children });
  });
}

// 创建表格并挂载
export async function createTableFromFetch(): Promise<void> {
  try {
    ...
    const columns = [
      ...
      },
      {
        ...
        style: {
          barHeight: '100%',
          barBgColor: (data: { percentile: number }) => {
            const greenValue = 255 * (1 - data.percentile);
            return `rgb(${200 + 50 * (1 - data.percentile)},${greenValue},${greenValue})`;
          },
      ...
      },
      {
        ...
        style: {
          showBar: true,
          barColor: (data: { percentile: number }) => {
            const greenValue = 255 * (1 - data.percentile);
            return `rgb(${200 + 50 * (1 - data.percentile)},${greenValue},${greenValue})`;
          },
          barHeight: 20,
    ...
    const option: VTable.ListTableConstructorOptions = {
      ...
      columns: columns as VTable.TYPES.ColumnsDefine,
    ...
    // attach for debug
    (window as unknown as { tableInstance?: VTable.ListTable }).tableInstance = tableInstance;
    (window as unknown as { masterDetailPlugin?: MasterDetailPlugin }).masterDetailPlugin = masterDetailPlugin;
...
// auto-create on script load
try {
  if (document.getElementById(CONTAINER_ID)) {
    // fire and forget
    void createTableFromFetch();
  }
} catch (e) {
  // ignore
}
