...
export function createTable() {
  ...
  fetch('https://lf9-dp-fe-cms-tos.byteorg.com/obj/bit-cloud/VTable/olympic-winners.json')
    ...
    .then(data => {
      const columns: VTable.ColumnDefine[] = [
        ...
        },
        {
          ...
          aggregation: {
            ...
            aggregationFun(values, records: any[]) {
              // 使用 reduce() 方法统计金牌数
      ...
      let lastFilterField;
      instance.on('icon_click', args => {
        const { col, row, name } = args;
        if (name === 'filter') {
          const field = instance.getHeaderField(col, row);
          if (select && lastFilterField === field) {
            removeFilterElement();
            lastFilterField = null;
          } else if (!select || lastFilterField !== field) {
            const rect = instance.getCellRelativeRect(col, row);
            createFilterElement(filterListValues[field as string], filterListSelectedValues, field, rect);
            lastFilterField = field;
          }
        }
      });

      let filterContainer = instance.getElement();
      let select;
      function createFilterElement(values: string[], curValue: string, field, positonRect) {
        // create select tag
