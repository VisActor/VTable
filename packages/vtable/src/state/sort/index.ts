...
export function dealSort(col: number, row: number, table: ListTableAPI, event: Event) {
  ...
  let sortState = Array.isArray(table.internalProps.sortState) ? table.internalProps.sortState : [table.internalProps.sortState];
  if(Array.isArray(sortState)){
    let index = sortState.findIndex(item=>item.field == tableState.field);
    if(index >= 0){
