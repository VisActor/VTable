...
export class ListTable extends BaseTable implements ListTableAPI {
   ...
   * @param dispatchOnValueChange 是否在值发生改变的时候触发change_cell_value事件
   */
  changeCellValue(
    col: number,
    row: number,
    ...
    return listTableChangeCellValue(col, row, value, workOnEditableCell, dispatchOnValueChange, this);
  }
  /**
   * 批量更新多个单元格的数据
   * @param col 粘贴数据的起始列号
  ...
  changeCellValues(
    ...
    return listTableChangeCellValues(startCol, startRow, values, workOnEditableCell, dispatchOnValueChange, this);
  }
  /**
   * 添加数据 单条数据
   * @param record 数据
