...
export interface IBaseTableProtected {
  ...
  /** 拖拽表头移动位置 针对冻结部分的规则
   * "disabled"（禁用）：不允许其他列的表头拖拽操作涉及到冻结列部分，冻结列保持不变。
   * "adjustCount"（调整数量）：允许其他列的表头拖拽操作涉及到冻结列部分，并根据拖拽的动作调整冻结列的数量。当其他列的表头被拖拽进入冻结列位置时，冻结列数量增加；当其他列的表头被拖拽移出冻结列位置时，冻结列数量减少。
   * "freeAdjust"（自由调整）：允许自由拖拽其他列的表头进入或移出冻结列位置，同时保持冻结列的数量不变。
   */
  frozenColDragHeaderMode?: 'disabled' | 'adjustCount' | 'freeAdjust';
