...
fruit and seeds. They are also highly intelligent and social, and are considered an intelligent, playful bird.`,
  ...
  test('listTable-cellType-function getAllRowsHeight', () => {
    expect(listTable.getAllRowsHeight()).toBe(960);
    expect(listTable.scenegraph.getCell(2, 1).children[0].type).toBe('image');
    expect(listTable.scenegraph.getCell(2, 2).children[0].type).toBe('text');
