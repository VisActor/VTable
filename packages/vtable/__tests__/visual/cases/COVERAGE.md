...
本清单描述当前本地用例的实际验证目的，具体 ID 用 `--list` 查询。一个视觉用例只代表所写的固定输入、动作和断言；不能据此推断相邻配置或所有 BugServer 用例已覆盖。
...
| 单元格与样式 | `wrapped-text`、`checkbox-states`、`mixed-sparkline-progress`、`switch-button`、`inline-image`、`style-underline` | 文本、复选框、火花线、进度条、开关、按钮、内联图片、下划线 |
| 布局与主题 | `auto-width-height`、`adaptive-width`、`layout-container-fit-height`、`layout-container-fit-both`、`frozen-short-content-connected`、`theme-dark-icons`、`theme-update-theme`、`layout-min-max-adaptive-width`、`pivot-max-frozen-width` | 自适应尺寸、冻结边界、深色图标、运行时主题更新、自适应列宽上下限、透视冻结宽度上限 |
| 透视表 | `pivot-analysis-two-level`、`pivot-virtual-column-total`、`pivot-calculated-field-hidden-dependencies`、`pivot-custom-merge-subtotal`、`pivot-indicator-sort-subtotal`、`pivot-aggregation-rules`、`pivot-tree-zero-expand`、`pivot-update-sort-state-click`、`pivot-initial-column-sort-state`、`pivot-custom-total-data-tree`、`pivot-corner-empty-matrix`、`pivot-empty-string-dimension`、`pivot-resize-indicator-row`、`pivot-corner-sort-one-dimension`、`pivot-corner-sort-two-dimensions` | 维度层级、虚拟节点、隐藏依赖指标、SUM/COUNT/AVG 聚合、指标路径排序、小计、初始列树排序状态与点击后更新排序状态、显式汇总记录、空维度矩阵、空字符串维度、指标行拖动调整行高、单级与两级角头初始排序图标 |
...
| 交互与组件 | `select-cross`、`keyboard-copy-paste-event`、`sort-multiple-header-click`、`edit-complete-input`、`component-legend-resize`、`menu-dropdown-highlight`、`hover-tooltip`、`drag-column-header`、`select-disable-callback`、`blank-area-deselect`、`disabled-arrow-navigation`、`drag-row-series-select`、`keyboard-cut-paste-event`、`resize-row-height` | 列拖动、回调禁选、选择、复制粘贴、复合排序、编辑、图例缩窄、下拉菜单、提示、空白区清选、禁选列键盘导航、行序号拖选、剪切粘贴、列表行高拖动 |
| 数据与 API | `aggregation-pagination`、`filter-api`、`records-add-sorted`、`records-delete-sorted`、`transpose-update-option-off`、`api-update-columns`、`records-add-pagination`、`promise-custom-merge`、`promise-keep-data` | 聚合、筛选、分页插入、异步数据自定义合并、排序状态下增删记录、转置更新、列更新、异步数据源配置更新时保留数据 |
...
上轮未选入的 528 条候选已按来源摘要、配置及 API 信号、录制动作逐条登记在 [去向台账](./BUGSERVER_CANDIDATE_TRIAGE.json)。状态为 `migrated` 的 20 条均指向实际运行的本地用例及断言；`deferred_*` 只表示尚未证明本地覆盖，不应据相似配置名或 AST 结构推断等价。

| 去向 | 数量 | 含义 |
| --- | ---: | --- |
| 已迁入本地用例 | 20 | 13 个新模块，以匿名数据重建配置及动作 |
| 隐私风险排除 | 7 | 发现业务化字段或数值，原始内容没有进入仓库 |
| 官方基线待查 | 1 | 初始排序状态存在，但预期记录顺序未改变 |
| 外部资源待替换 | 31 | 需要确定性的本地资源后才能验证 |
| 大型源码待缩减 | 70 | 需要缩成匿名且仍保留关键边界的数据 |
| 交互未证明覆盖 | 176 | 录制动作缺少对应的本地动作与断言 |
| 静态配置未证明覆盖 | 223 | 配置或数据变体缺少对应的本地断言 |
