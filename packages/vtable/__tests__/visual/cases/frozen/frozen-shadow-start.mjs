/**
 * BugServer case IDs: 6aaba81349fc4f005c341b4c
 * 验证目的：横向滚动到 start 时，冻结列阴影仅在存在溢出的一侧显示。
...
export default {
  ...
  async exercise(page) {
    ...
    await page.evaluate(() => window.__visualTable.setScrollLeft(0));
    const host = await page.locator('#table').boundingBox();
    await page.mouse.move(host.x + 280, host.y + 100);
  ...
  async verify(page) {
    ...
    }, { left: false, right: true, position: 'start' });
  }
};
