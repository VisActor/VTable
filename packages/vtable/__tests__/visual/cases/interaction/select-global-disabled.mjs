/**
 * BugServer case IDs: 658d566a20ffa2008c33ebf1
 * 验证目的：全局 disableSelect 阻止表体和表头点击生成选择范围。
...
export default {
  ...
  async verify(page) {
    ...
    await page.evaluate(() => {
      ...
      if (table.getSelectedCellRanges().length) throw new Error('全局禁选仍产生选择范围');
      if (!table.options.select?.disableSelect) throw new Error('全局禁选配置未应用');
    });
  }
};
