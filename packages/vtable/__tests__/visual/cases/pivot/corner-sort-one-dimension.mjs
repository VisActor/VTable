/**
 * BugServer case IDs: 69d5d35be48f3a005ed8b838
 * 验证目的：角头启用排序图标时，透视排序状态可指向单级角头维度。
...
export default {
  mount(container) {
    ...
    return new window.VTable.PivotTable(container, {
      ...
      pivotSortState: [{ dimensions: [{ dimensionKey: 'category', isPivotCorner: true }], order: 'desc' }]
    });
  },
  async verify(page) {
    ...
    await page.evaluate(() => {
      ...
      if (state?.order !== 'desc' || state.dimensions?.length !== 1 ||
